        ...
        for model in gpt-4o-mini; do
            for split in test_hard; do #sample_1_hard, test_hard
                for difficulty in adaptive; do
                    adaptive_rag_tag=$(echo $adaptive_rag | tr ' ' '_')
                    mkdir -p $LOGS_DIR/$dataset/run_$RUN_ID
                    log_file=$LOGS_DIR/$dataset/run_${RUN_ID}/${model}_${dataset}_${split}_${difficulty}_${adaptive_rag_tag}.log
                    error_file=$LOGS_DIR/$dataset/run_${RUN_ID}/${model}_${dataset}_${split}_${difficulty}_${adaptive_rag_tag}.err
                    echo "Running $model on $split with difficulty $difficulty, adaptive_rag $adaptive_rag"
                    python main.py \
                    --model_name $model \

...
#        print(output + "\n")
    ...
    def process_query(self, initial_query: str) -> Dict[str, Any]:
        ...
        while iteration < max_iterations:
            if iteration == 0:
                ...
                retrieval_query = f"Original Query: {initial_query} \n\n Previous Report: {consensus_result}"
            expert_analyses = []
            for role in self.expert_roles:
                queries = self._generate_expert_query(role, retrieval_query)
                retrieved_docs = self._retrieve_queries(queries)
                expert_analyses.append(self._expert_analysis(role, retrieved_docs, retrieval_query))
    ...
    def _generate_expert_query(self, role, context: str) -> List[str]:
        """Generate expert queries based on context."""
        query_prompt = """Generate up to three specific medical queries addressing:

        {context}

        Consider: Additional information needed, Remaining knowledge gaps, Expert disagreements
    ...
    def _expert_analysis(self, role, documents_or_knowledgebase: List[str], query: str) -> List[Dict]:
        """Perform expert analysis on the current knowledge base."""
        analysis_prompt = f"""Analyze the document and solve the query:

        Document: {self._format_docs_for_prompt(documents_or_knowledgebase)}
        ...
        return {'role': role, 'analysis': self._clean_text(self._call_llm([
                {"role": "system", "content": f"You are a {role} specialist analyzing medical information and solving medical question."},

...
Passage:"""
    ...
    def _filter_docs(self, query: str, retrieved_docs: List[str], model) -> List[str]:
        ...
        for doc in retrieved_docs:
            ...
            try:
                ...
                if "ully" in response.lower():
                    fully_relevant_docs.append(doc)
                elif "artially" in response.lower():
                    partially_relevant_docs.append(doc)
            except Exception as e:
                print(f"Error filtering document: {str(e)}")
                continue
        if fully_relevant_docs:
            return fully_relevant_docs
        else:
            return partially_relevant_docs
    ...
    def _final_answer_pick(self, query, text):
        return self._call_llm([
            {
                "role": "system", 
                "content": "You are an answer parser. Pick an answer even if there is no consensus."
            },
            {
                "role": "user", 
                "content": f'Only output A, B, C, or D written in the context: {text}\n\n options list: {query.split("Options:")[-1]}'
            }
        ], self.model, temperature=0)

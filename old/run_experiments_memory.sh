        ...
        for model in gpt-4o-mini; do
            for split in test_hard; do #sample_1_hard, test_hard
                for difficulty in adaptive; do
                    agent_memory_tag=$(echo $agent_memory | tr ' ' '_')
                    mkdir -p $LOGS_DIR/$dataset/run_$RUN_ID
                    log_file=$LOGS_DIR/$dataset/run_${RUN_ID}/${model}_${dataset}_${split}_${difficulty}_${agent_memory_tag}.log
                    error_file=$LOGS_DIR/$dataset/run_${RUN_ID}/${model}_${dataset}_${split}_${difficulty}_${agent_memory_tag}.err
                    echo "Running $model on $split with difficulty $difficulty, agent_memory $agent_memory"
                    python main.py \
                    --model_name $model \

"""Discussion Patterns PDF Display - Figure 1"""
...
def plot_discussion_patterns_pdf(ax, pdf_path=None, panel_label='G'):
    """Display the discussion patterns PDF as an image"""
    
    if pdf_path is None:
        # Default path to discussion_patterns.pdf in same directory
        current_dir = os.path.dirname(os.path.abspath(__file__))
        pdf_path = os.path.join(current_dir, 'discussion_patterns.pdf')
    ...
    try:
        ...
        if images:
            ...
            # Display the image with original aspect ratio (wide format)
            ...
            print(f"Successfully loaded discussion patterns PDF from {pdf_path}")
        else:
            _create_placeholder_figure(ax, panel_label)
...
def _create_placeholder_figure(ax, panel_label):
    ...
    ax.text(0.5, 0.5, 'Discussion Patterns\n(PDF Display)', 
...
def main():
    ...
    plot_discussion_patterns_pdf(ax, panel_label='G')
    plt.tight_layout()
    plt.savefig('discussion_patterns_display_example.pdf', dpi=300, bbox_inches='tight')
    plt.show()

...
def plot_triage_pdf(ax, pdf_path=None, panel_label='F'):
    """Display the triage PDF as an image"""
    
    if pdf_path is None:
        # Default path to triage.pdf in same directory
        current_dir = os.path.dirname(os.path.abspath(__file__))
        pdf_path = os.path.join(current_dir, 'triage.pdf')
    ...
    try:
        ...
        if images:
            ...
            # Display the image with original aspect ratio (square-ish)
            ...
            print(f"Successfully loaded triage PDF from {pdf_path}")
        else:
            _create_placeholder_figure(ax, panel_label)
...
def _create_placeholder_figure(ax, panel_label):
    ...
    ax.text(0.5, 0.5, 'Triage Analysis\n(PDF Display)', 
...
def main():
    ...
    plot_triage_pdf(ax, panel_label='F')
    plt.tight_layout()
    plt.savefig('triage_display_example.pdf', dpi=300, bbox_inches='tight')
    plt.show()

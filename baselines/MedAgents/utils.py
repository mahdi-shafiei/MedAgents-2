...
def fully_decode(qid, realqid, question, options, gold_answer, handler, args, dataobj):
    ...
    if args.method == "base_direct":
        ...
        output = handler.get_output_multiagent(user_input=direct_prompt, temperature=0, system_role="")
        ans, output = cleansing_final_output(output)
    elif args.method == "base_cot":
        cot_prompt = get_cot_prompt(question, options)
        output = handler.get_output_multiagent(user_input=cot_prompt, temperature=0, system_role="")
        ans, output = cleansing_final_output(output)
    else:
        ...
        for _domain in question_domains:
            ...
            raw_question_analysis = handler.get_output_multiagent(user_input=prompt_get_question_analysis, temperature=0, system_role=question_analyzer)
        ...
        for _domain in options_domains:
            ...
            raw_option_analysis = handler.get_output_multiagent(user_input=prompt_get_options_analyses, temperature=0, system_role=option_analyzer)
        ...
        if args.method == "anal_only":
            ...
            output = handler.get_output_multiagent(user_input=answer_prompt, temperature=0, system_role="")
            ans, output = cleansing_final_output(output)
        else:
            ...
            raw_synthesized_report = handler.get_output_multiagent(user_input=prompt_get_synthesized_report, temperature=0, system_role=synthesizer)
            ...
            if args.method == "syn_only":
                ...
                while num_try < args.max_attempt_vote and hasno_flag:
                    ...
                    for domain in all_domains:
                        ...
                        raw_domain_opi = handler.get_output_multiagent(user_input=cons_prompt, temperature=0, system_role=voter)
                        ...
                        if domain_opinion == "no":
                            ...
                            advice_output = handler.get_output_multiagent(user_input=advice_prompt, temperature=0, system_role=voter)
                            revision_advice[domain] = advice_output
                            hasno_flag = True
                    if hasno_flag:
                        revision_prompt = get_revision_prompt(syn_report, revision_advice)
                        revised_analysis = handler.get_output_multiagent(user_input=revision_prompt, temperature=0, system_role="")

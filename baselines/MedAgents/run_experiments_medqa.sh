...
for model in gpt-4o-mini gpt-4o; do
    for split in sampled_50_hard sampled_50 sampled_50_5options; do
        for difficulty in adaptive; do
            ...
            python main.py --dataset_name medqa --dataset_dir ./data/medqa/ --split $split --model $model --method syn_verif --output_files_folder ./output/ --num_processes 4 > $log_file 2> $error_file
        done
    done
done
